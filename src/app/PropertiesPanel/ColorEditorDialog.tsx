  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title='Edit Colors'>
      <div className='mt-4 w-[520px] text-[12px]'>
        <div className='mb-2 grid grid-cols-[minmax(0,1fr)_repeat(4,54px)_28px] gap-1 px-1 text-[10px] uppercase text-[#8f8f8f]'>
          <span>Name</span><span>R</span><span>G</span><span>B</span><span>A</span><span />
        </div>
        <div className='max-h-72 space-y-1 overflow-y-auto'>
          {colors.map((color, colorIndex) => (
            <div className='grid grid-cols-[minmax(0,1fr)_repeat(4,54px)_28px] gap-1' key={color.originalKey || `${color.key}-${colorIndex}`}>
