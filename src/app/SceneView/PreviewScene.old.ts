...
import { LabelComp, loadAll, NodeComp, SceneComponent, SpriteRender, Touch } from '@safe-engine/webgl'
import { SpineBonesControlComponent, SpineSkeleton } from '@safe-engine/webgl/dist/spine'
...
export class PreviewScene extends SceneComponent {
  ...
  boxColliderEditorNode?: NodeComp
  marqueeSelectionNode: NodeComp
  spineBonesControlNode: NodeComp
  ...
  async loadProjectData() {
    ...
    if (Number.isFinite(defaultFontSize) && defaultFontSize > 0) LabelComp.defaultSize = defaultFontSize
  ...
  createBorder() {
    ...
    border.w = designedResolution.width
    border.h = designedResolution.height
    border.anchorX = 0
    border.anchorY = 0
  ...
  createArrows() {
    ...
    arrowSpriteHorizon.addComponent(new SpriteRender({ spriteFrame: arrow }))
    arrowSpriteVertical.addComponent(new SpriteRender({ spriteFrame: arrow }))
  ...
  getSpineBonesControl() {
    ...
    const componentIndex = editNode?.components?.findIndex((component) => component.tag === 'SpineBonesControlComponent') ?? -1
  ...
  moveSpineBonePoint(x: number, y: number) {
    ...
    const liveControl = currentNode.getComponent(SpineBonesControlComponent)
